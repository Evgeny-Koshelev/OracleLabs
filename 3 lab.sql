...
--  организации).
select  connect_by_root(e.employee_id) as e_id,
        connect_by_root(e.first_name || ' ' || e.last_name) as e_name,
        e.employee_id as m_id,
        e.first_name || ' ' || e.last_name as m_name,
        level - 2 as m_count
  from  employees e
  where level > 1
...
select  o.customer_id as c_id,
        substr(sys_connect_by_path(to_char(o.o_date2, 'DD.MM.YYYY'), ', '), 3) as o_dates
  from  (
          select  o.customer_id,
                  ...
                  o.order_date as o_date2
            from  orders o
            group by  o.customer_id,
                      o.order_date
          ) o
  where connect_by_isleaf = 1
  start with  o.o_date1 is null
  connect by  o.customer_id = prior(o.customer_id) and
              o.o_date1 = prior(o.o_date2)
;
...
select  o.customer_id as c_id,
        listagg(to_char(o.order_date, 'DD.MM.YYYY'), ', ')
          within group (order by o.order_date) as o_dates
  from  orders o
  group by  o.customer_id
;
...
select  e.employee_id as e_id,
        ...
        c.cust_first_name || ' ' || c.cust_last_name as c_name,
        o.order_date as o_date,
        o.order_total as o_total,
        (
  ...
  order by  o_date desc nulls last,
            o_total desc nulls last,
            e_id
;
...
holidays as 
(
  select date'2021-01-01' as dt, 1 as comments from dual union all
  select date'2021-01-04', 1 from dual union all
  select date'2021-01-05', 1 from dual union all
  select date'2021-01-06', 1 from dual union all
  select date'2021-01-07', 1 from dual union all
  select date'2021-01-08', 1 from dual union all
  select date'2021-02-20', 0 from dual union all
  select date'2021-02-22', 1 from dual union all
  select date'2021-02-23', 1 from dual union all
  select date'2021-03-08', 1 from dual union all
  select date'2021-05-03', 1 from dual union all
  select date'2021-05-10', 1 from dual union all
  select date'2021-06-14', 1 from dual union all
  select date'2021-11-04', 1 from dual union all
  select date'2021-11-05', 1 from dual union all
  select date'2021-12-31', 1 from dual
)
