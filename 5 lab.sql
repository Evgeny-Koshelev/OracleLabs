...
-- specialties
insert all
  into specialities (specialty_name) values ('Факультет компьюьерных наук')
  into specialities (specialty_name) values ('Физический факультет')
  into specialities (specialty_name) values ('ПММ')
select * from dual
;

-- subjects
insert all
  into subjects (subject_name) values ('Математический анализ')
  into subjects (subject_name) values ('Введение в программирование')
  into subjects (subject_name) values ('Иностранный язык')
  into subjects (subject_name) values ('История')
  into subjects (subject_name) values ('Основы речевого воздействия')
select * from dual;

-- students
insert all
  into students (first_name, last_name, specialty_id, receipt_year) values ('Алексей', 'Кошелев', 1, 2017)
  into students (first_name, last_name, specialty_id, receipt_year) values ('Александр', 'Егоров', 3, 2019)
  into students (first_name, last_name, specialty_id, receipt_year) values ('Анна', 'Скачкова', 2, 2017)
select * from dual;
...
insert all
  ...
  into syllabuses (specialty_id, semester) values (3, 1)
  into syllabuses (specialty_id, semester) values (3, 2)
  into syllabuses (specialty_id, semester) values (3, 3)
  into syllabuses (specialty_id, semester) values (3, 4)
  into syllabuses (specialty_id, semester) values (3, 5)
  into syllabuses (specialty_id, semester) values (3, 6)
  into syllabuses (specialty_id, semester) values (3, 7)
  into syllabuses (specialty_id, semester) values (3, 8)
select * from dual;
...
--marks
insert all
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (2, 3, date'2021-02-27', 1, 1)
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (4, 2, date'2021-04-05', 9, 3)
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (5, 2, date'2021-03-21', 9, 4)
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (3, 1, date'2021-03-21', 1, 1)
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (5, 1, date'2021-02-15', 1, 2)
  into marks (mark, student_id, receiving_date, syllabus_id, subject_id) values (null, 3, date'2021-04-17', 1, 2)
select * from dual;
...
--  (сколько семестров проучился студент вычислять из года поступления 
--  и текущей даты – написать для этого функцию). Должны выводиться поля: код студента, 
--  ФИО студента, курс, код предмета, название предмета, семестр, оценка 
--  (2 – если сдавал экзамен, нулл – если не сдавал).
...
select  ct.student_id,
        ct.name, 
        count(ct.student_id) as tails_count
  from  count_tails_view ct
  group by  ct.student_id,
            ct.name
  having count(ct.student_id) > 3
;
